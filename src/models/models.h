...
struct llama_model_hyv4 : public llama_model_base {
    llama_model_hyv4(const struct llama_model_params & params) : llama_model_base(params) {}
    void load_arch_hparams(llama_model_loader & ml) override;
    void load_arch_tensors(llama_model_loader & ml) override;
    ...
    struct graph : public llm_graph_context {
        ...
        // iHC (independent Hyper-Connections): same layout as DeepSeek-V4 HC but without the
        // comb/sinkhorn term (see hyv4_plan/06). pre reduces the hc streams to one and returns
        // the per-stream post gates; post writes the sublayer output back into the streams;
        // head collapses the streams before the final norm.
        ggml_tensor * build_hc_pre(
                ggml_tensor * x,
                ggml_tensor * hc_fn,
                ggml_tensor * hc_scale,
        ...
        // it; "shared" layers reuse the last preceding full layer's result via last_top_k.
