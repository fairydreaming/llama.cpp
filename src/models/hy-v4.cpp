...
// iHC (independent Hyper-Connections) helpers. Same layout as DeepSeek-V4 HC but WITHOUT the
// comb/sinkhorn term: hc_fn produces only 2*hc coefficients (pre + post), the width mixing is
// implicit in the pre-reduce / post-distribute round trip. See hyv4_plan/06 Delta 3.

static size_t hyv4_elem_offset(const ggml_tensor * t, int64_t i) {
    return ggml_row_size(t->type, i);
}

static ggml_tensor * hyv4_view_1d(ggml_context * ctx, ggml_tensor * t, int64_t ne0, int64_t i0) {
    return ggml_view_1d(ctx, t, ne0, hyv4_elem_offset(t, i0));
}
...
static ggml_tensor * hyv4_hc_reduce(ggml_context * ctx0, ggml_tensor * x, ggml_tensor * w, int64_t hc, int64_t n_embd, int64_t nt, ggml_type out_type) {
...
ggml_tensor * llama_model_hy_v4::graph::build_hc_pre(
    ...
    ggml_tensor * scale_pre  = hyv4_view_1d(ctx0, hc_scale, 1, 0);
    ggml_tensor * scale_post = hyv4_view_1d(ctx0, hc_scale, 1, 1);
    ggml_tensor * base_pre   = hyv4_view_1d(ctx0, hc_base, hc, 0);
    ggml_tensor * base_post  = hyv4_view_1d(ctx0, hc_base, hc, hc);

    // pre = sigmoid(mixes[:hc]*scale_pre + base_pre) + eps
    ggml_tensor * pre = hyv4_view_2d(ctx0, mixes, hc, nt, 0);
    ...
    ggml_tensor * po = hyv4_view_2d(ctx0, mixes, hc, nt, hc);
...
ggml_tensor * llama_model_hy_v4::graph::build_indexer_top_k(
    ...
    // nope first, pe last: the reference applies the standard rotate_half (ggml NEOX) with no
    ...
    // the FP8 kernels (see hyv4_plan/10 section 3.5.4)
